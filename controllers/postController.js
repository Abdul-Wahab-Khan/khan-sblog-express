...
const postPost = asyncHandler(async (req, res) => {
    ...
    const post = await Post.create({
        title, body, user_id: req.user.id, imageUrl
    })
...
const unlike = asyncHandler(async (req, res) => {
    const post_id = req.body.id
    const user_id = req.user.id 
    const post = await Like.find({$and: [{user_id: user_id, post_id: post_id}]})
    if (post) {
...
module.exports = {
    getPosts, getPost, postPost, putPost, deletePost, getUserPosts, markApproved, markDisApproved,
    like, unlike, isLiked
}